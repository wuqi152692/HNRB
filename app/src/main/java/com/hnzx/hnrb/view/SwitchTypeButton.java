...
import com.hnzx.hnrb.tools.ColorUtils;
...
public class SwitchTypeButton extends CompoundButton {
    /**
     * 默认计算圆角
     */
    ...
    public SwitchTypeButton(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs);
    }

    public SwitchTypeButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public SwitchTypeButton(Context context) {
        super(context);
        init(null);
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        //加载自定义属性值
        TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R.styleable.SwitchTypeButton);
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.SwitchTypeButton_ThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.SwitchTypeButton_ThumbColor);
            margin = ta.getDimension(R.styleable.SwitchTypeButton_ThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.SwitchTypeButton_ThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.SwitchTypeButton_ThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.SwitchTypeButton_ThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.SwitchTypeButton_ThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.SwitchTypeButton_ThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.SwitchTypeButton_ThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.SwitchTypeButton_ThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.SwitchTypeButton_BackRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.SwitchTypeButton_BackDrawable);
            backColor = ta.getColorStateList(R.styleable.SwitchTypeButton_BackColor);
            backMeasureRatio = ta.getFloat(R.styleable.SwitchTypeButton_BackMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.SwitchTypeButton_AnimationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.SwitchTypeButton_FadeBack, true);
            tintColor = ta.getColor(R.styleable.SwitchTypeButton_TintColor, tintColor);
            textOn = ta.getString(R.styleable.SwitchTypeButton_TextOn);
            textOff = ta.getString(R.styleable.SwitchTypeButton_TextOff);
            textMarginH = Math.max(textMarginH, backRadius / 2);
            textMarginH = ta.getDimension(R.styleable.SwitchTypeButton_TextMarginH, textMarginH);
            autoAdjustTextPosition = ta.getBoolean(R.styleable.SwitchTypeButton_AutoAdjustTextPosition, autoAdjustTextPosition);
            isSameColor = ta.getBoolean(R.styleable.SwitchTypeButton_IsSameColor, isSameColor);
            ta.recycle();
        }
        ...
        if (!mIsBackUseDrawable && mBackColor == null) {
            ...
            if (isSameColor) {
                mBackColor = ColorUtils.generateSameColorByTintColor(mTintColor);
            } else {
                mBackColor = ColorUtils.generateBackColorByTintColor(mTintColor);
            }
    ...
    private void setup() {
        ...
        if (mOnLayout != null && mOffLayout != null) {
            if (mThumbMargin.top + mThumbMargin.bottom > 0) {
                float addition = (getMeasuredHeight() - getPaddingBottom() -
                        getPaddingTop() - mThumbSizeF.y - mThumbMargin.top - mThumbMargin.bottom) / 2;
                thumbTop += addition;
            }
        }
        ...
        if (mOnLayout != null) {
            float marginOnX = mBackRectF.left + (mBackRectF.width() - mThumbRectF.width() - mThumbMargin.right - mOnLayout.getWidth()) / 2
                    + (mThumbMargin.left < 0 ? mThumbMargin.left * -0.5f : 0);
        ...
        if (mOffLayout != null) {
            float marginOffX = mBackRectF.right - (mBackRectF.width() - mThumbRectF.width() - mThumbMargin.left - mOffLayout.getWidth()) / 2
                    - mOffLayout.getWidth() + (mThumbMargin.right < 0 ? mThumbMargin.right * 0.5f : 0);
            if (!mIsBackUseDrawable && mAutoAdjustTextPosition) {
